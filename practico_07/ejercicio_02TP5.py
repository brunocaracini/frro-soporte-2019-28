...
class DatosSocio(object):
    ...
    def modificacion(self, socio_mods, id_socio):
        socio = self.session.query(Socio).filter(Socio.id_socio == id_socio)
        if socio == None:
            return False
        else:
            self.baja(id_socio)
            socio_mods.id_socio = id_socio
            return self.alta(socio_mods)
...
def pruebas():
    ...
    datos.modificacion(socio_3,socio_3.id_socio)
